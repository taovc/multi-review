...
On first start, **the SQLite database (`./data/cockpit.db`) is created automatically**. The default worktree location is `pr-cockpit-worktrees/` inside each project's local clone; startup recovery moves existing persistent worktrees from the old `./data/worktrees` location when they still exist. No manual migration is required. The Drizzle schema is set up on the fly (`ensureSchema()` / `ensureColumns()` in `core/db/client.ts`).
...
| `WORKTREE_LOCATION` | `repo` | `repo` = `pr-cockpit-worktrees/` inside each local clone; `central` = use `REPOS_DIR` |
