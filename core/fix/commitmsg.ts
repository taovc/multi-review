...
export async function genCommitMessage(model: string, diff: string, cwd: string): Promise<string> {
...
${clipped}`
  try {
    const out = await runHelperText({ prompt, cwd, model: model || 'sonnet', effort: 'low', timeoutMs: 120_000 })
