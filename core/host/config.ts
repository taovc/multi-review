...
// Agent configuration transparency: which files Claude Code loads for a given cwd (presence and size only — settings
// contents are never read here, their env values may hold secrets) and what the CLI actually reports once started
// (commands, MCP status, startup context). Trimmed on purpose (2026-08): merged-settings provenance, the per-run-kind
// override table, account / models / agents were never looked at.

export type Scope = 'user' | 'project' | 'local' | 'managed'
export type ConfigFile = { path: string; kind: 'settings' | 'memory' | 'rules' | 'skills' | 'commands' | 'plugins' | 'mcp'; scope: Scope; exists: boolean; bytes: number; count: number | null }
...
export async function probeAgent(cwd: string, chrome: boolean, refresh = false): Promise<ProbeReport> {
  ...
  const p = (async (): Promise<ProbeReport> => {
    ...
    const report: ProbeReport = { cwd, at: new Date().toISOString(), ms: 0, error: null, commands: [], mcp: [], context: [], chromeTransport: chrome ? 'extraArgs' : 'off' }
...
export async function agentConfigReport(opts: { cwd: string; agent: AgentSettings; probe: boolean; refresh?: boolean }): Promise<AgentConfigReport> {
  ...
  return { cwd, projectDirName: projectDirNameFor(cwd), files: listConfigFiles(cwd), probe, codex, agent: opts.agent }
}
