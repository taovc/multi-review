...
// A cockpit-side command shadows the CLI's built-in of the same name (/model, /effort): one row, ours.
const catalogue = computed(() => { const local = new Set(props.local.map((e) => e.name)); return props.entries.filter((e) => !local.has(e.name)) })
const filtered = computed<CommandEntry[]>(() => {
  if (props.head == null) return []
  const h = props.head
  return [...filterCommands(props.local, h, true), ...filterCommands(catalogue.value, h, showAll.value)]
})
defineExpose({ filtered })

type Group = { key: string; label: string; items: CommandEntry[] }
const groups = computed<Group[]>(() => {
  const all = showAll.value ? catalogue.value : catalogue.value.filter((e) => e.curated)
...
const hiddenCount = computed(() => catalogue.value.filter((e) => !e.curated).length)
...
<template>
  ...
  <div v-if="head != null && filtered.length" ref="listEl" class="absolute bottom-full left-0 mb-1 w-full max-h-72 overflow-y-auto bg-default border border-default rounded shadow-lg z-10 text-xs">
    <div
      v-for="(c, i) in filtered" :key="c.origin + ':' + c.name" :data-i="i"
  ...
  <div v-if="browse" class="absolute bottom-full left-0 mb-1 w-full max-h-96 overflow-y-auto bg-default border border-default rounded shadow-lg z-10 text-xs">
    ...
    <div v-for="g in groups" :key="g.key">
      ...
      <div v-for="c in g.items" :key="c.origin + ':' + c.name" class="flex items-center gap-3 px-3 py-1 hover:bg-muted cursor-pointer" :title="c.name" @mousedown.prevent="emit('pick', c)">
