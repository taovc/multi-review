...
import { resultOwner } from '../core/host/normalize'
...
const { claudeHost } = await import('../core/host/claudeHost')
...
assert.equal(resultOwner(done({ numTurns: 0, resultText: '' , isError: true, subtype: 'error_during_execution' }), { uuid: 'U1', activity: false }), 'ours', 'a session-scoped failure names no send and still ends our turn')
assert.equal(resultOwner(done(), { uuid: 'U1', activity: false }), 'ours', 'an older CLI sends no uuid: a real result still ends the turn')
assert.equal(resultOwner(metaResult, { uuid: null, activity: false }), 'ours', 'no uuid on our side: nothing to match, behave as before')
assert.equal(resultOwner(metaResult, { uuid: 'U1', activity: false, interrupted: true }), 'ours', 'an interrupted turn ends on the next result, empty or not')
...
// 2) A result naming a different send never closes ours, however long it waits.
{
  const live = liveRun()
  startTurn(live, 'U-ours')
  dispatch(live, done({ userMessageUuid: 'U-other', resultText: 'someone else' }))
  assert.equal(live.resolved, null)
  assert.equal(live.orphanTimer, null, 'a positively foreign result needs no fallback')
  await new Promise((r) => setTimeout(r, 120))
  assert.equal(live.resolved, null, 'still open')
...
// 5) Stop must not wait on the fallback: an interrupted turn ends on whatever result comes back.
{
  const live = liveRun()
  startTurn(live, 'U-ours')
